...
impl BookItem {
    ...
    pub fn check_validity(&self) -> bool {
        match self {
            BookItem::Book { pages, discount } => pages > 0 && match discount {
                Some(d) => d <= 50,
                None => true,
            },
            BookItem::EBook(name, (_, sc)) => !name.is_empty() && *sc > 0,
            BookItem::Collection(items) => !items.is_empty() && items.iter().all(|item| item.check_validity()),
