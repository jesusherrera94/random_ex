...
pub fn find_all_indices(s: &str, pattern: &str) -> Vec<usize> {
    ...
    s.match_indices(pattern).map(|(index, _)| index).collect()
}
...
pub fn extract_between(s: &str, start: &str, end: &str) -> Option<String> {
    ...
    let start_at = s.find(start)?;
    let content_at = start_at + start.len();
    let end_at = s[content_at..].find(end)? + content_at;
    Some(s[content_at..end_at].to_string())
}
